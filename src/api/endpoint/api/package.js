...
module.exports = function (route, auth, storage, config) {
	...
	route.get('/:package/:version?', can('access'), function (req, res, next) {
		const getPackageMetaCallback = function (err, info) {
			if (err) {
				if (err.status === 404) {
					let masterPackage = req.params.package.substring(req.params.package.indexOf("branch-")+7);
					console.log("req.params.package= "+ req.params.package.substring(req.params.package.indexOf("branch-")+7));
					storage.get_package({
						name: masterPackage,
						req,
